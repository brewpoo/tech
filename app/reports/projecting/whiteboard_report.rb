class Projecting::WhiteboardReport < BaseReport
  ...
  def setup
    ...
    data.reorder("project_number", "title", "requested_on", "started_on", "estimated_completion", "completed_on", "priority", "manager",
      "lead_engineer", "department", "requestor", "project_status", "total_spent", "latest_update") unless data.length == 0
    data.column_names = data.column_names.collect{ |cn| cn.gsub(/\./, ' ').titleize }
  end

  formatter :html do
    build :list do
      output << "<h3>Whiteboard Report</h3>"
  ...
  formatter :pdf do
    build :list do
      pad(10) { add_text "Whiteboard Report" }
